...
class Tokenizer(nn.Module):
    ...
    def forward(self, x_num: Tensor, x_cat: ty.Optional[Tensor]) -> Tensor:
        ...
        # Add [CLS] token for numerical features
        x_num = torch.cat(
        ...
        if x_cat is not None:
            x_cat_int = x_cat.int()
            x = torch.cat(
                [x, self.category_embeddings(x_cat_int + self.category_offsets[None])],
                dim=1,
            )
...
class UNETR(nn.Module):
    ...
    def __init__(self,
        ...
        self.patch_embedding = PatchEmbeddingBlock(
            ...
            d_numerical=9,
            categories=[2,12,3,8,8,4,2,3,6,7,6,11],
        )
        ...
        fc_layers = [[nn.Linear(768 * 2, 256 * 2), 
        ...
        self.mtlr = nn.Sequential(*fc_layers, MTLR(64 * 2, 4),)
...
class PatchEmbeddingBlock(nn.Module):
    ...
    def __init__(
        ...
        self.n_patches = np.prod([im_d // p_d for im_d, p_d in zip(img_size, patch_size)]) + 22  # +22 for EHR
    ...
    def forward(self, x):
        ...
        clin_var = self.tokenizer(clin_var[:,:9], clin_var[:,9:])
...
if __name__ == '__main__':
    ...
    ta = pd.read_csv("D:\AProjection\SurvivalPrediction\data\preprocessed\clinal_ft.csv").iloc[:4,1:22].values
