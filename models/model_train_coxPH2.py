...
class TrainerCoxPH():
    ...
    def test(self):
        ...
        events = pd.read_csv(Path(self.dataset_path) / self.args.tabular_name).iloc[:, -1].tolist()
        
        if len(self.args.intervals) == 0:
            times = pd.read_csv(Path(self.dataset_path) / self.args.tabular_name).iloc[:, -2].tolist()
